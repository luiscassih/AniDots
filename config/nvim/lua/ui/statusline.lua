...
local animotion = require("AniMotion")
...
local function path_widget(root, fname)
  ...
  if remote and branch then
    ...
    repo_info = string.format("%s %s %s %s ", ICON.branch, remote, ICON.file, branch)
  end
  ...
  return repo_info .. ICON.file .. " " .. tools.hl_str(hl, icon) .. " " .. dir_path .. path .. " "
end
...
local function diagnostics_widget()
  ...
  local info = ""
  if diag_count[1] then info = info .. ICON.error .. " " .. tools.hl_str("DiagnosticError", string.format("%-3d", diag_count[1] or 0)) end
  if diag_count[2] then info = info .. ICON.warn .. " " .. tools.hl_str("DiagnosticWarn", string.format("%-3d", diag_count[2] or 0)) end
  return info
...
-- Noice statusline ---------------------------------------------
local function noice_statusline()
  if (noice.api.statusline.mode.has()) then
    return "%#DiagnosticError#" .. noice.api.statusline.mode.get() .. "%*"
  end
  return ""
end
